...
pro getpdf,xval,pval,res,err1,err2,xax,yax,pl=pl,log=log,no=no,fixed=fixed,leg,$
truth=truth,samp=samp
...
; if likelihood array contains only a small amout of non-zero values,
; assume colors are bogus 
; NB this cannot be fractional in JHK, since sometimes there are a small
; amount of M dwarfs compared to the red clump
...
if (float(n_elements(where(lh3 gt 0.01)))/float(n_elements(lh3)) lt 0.1) or $
